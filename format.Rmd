...
# set some colours
cols <- c("quarantine" = "#d7191c", "nursery" = "#fdae61", "juvenile" = "#abd9e9", "growout" = "#2c7bb6")
...
# 95^th^ st. 2022 Operation modelling and review
...
# custom color
system_cols <- c(quarantine = "#d7191c", nursery = "#fdae61", juvenile = "#ffffbf", growout = "#abd9e9", depurate = "#2c7bb6")
...
tss.c1.st <- desired.tss
tss.cbest.st <- 0
tss.te.st <- 0.24 
tss.c2.st = tss.c1.st + tss.te.st * (tss.cbest.st - tss.c1.st)
```
...
Suggested minimum quarantine biomedia surface areas are as follows (based on a 24 hr metabolic period and clearwater operating system):
...
The chart below indicates theoretical carbohydrate additions required to supplement organic carbon supplied in feed (nursery/ juvenile/ grow-out stages only). 
...
Flowrate in settling tank $V = Q/A$: $V = 1.8 / (pi * (0.584)^2) / 60^2$
V = `r 1.8 / (pi * (0.584)^2) / 60^2`m/s or `r 1.8 / (pi * (0.584)^2) / 60^2 * 100` cm/s

``` {r SettlingVelocities}
s.tank.v = 1.8 / (pi * (0.584)^2) / 60^2

settling.velocities <- tibble(particle.size = c(48, 75, 100, 250)) %>%
  mutate(settling.velocity.ms = 9.81 * (particle.size * 1e-6)^2 * (1050 - 1018) / (18 * 0.851e-3),
         pc.efficiency = settling.velocity.ms / s.tank.v * 100,
         pc.efficiency = case_when(pc.efficiency > 100 ~ 100, TRUE ~ pc.efficiency))
```

``` {r SettlingEfficiency, ft.align = "center"}

settling.velocities %>%
  flextable() %>%
  set_header_labels(particle.size = "Particle Size mu m", settling.velocity.ms = "Settling Velocity m/s ", pc.efficiency = "Percentage Efficiency") %>%
  colformat_double(digits = c(5)) %>%
  autofit() %>%
  fit_to_width(7.5) %>%
set_caption(caption = as_paragraph("95", as_sup("th"), " st. 2022 Particle settling efficiency at operational flowrate of settling tank."), 
  style = "Table Caption", 
  autonum = run_autonum(seq_id = "tab"))
```
Approximation for settling tank efficiency $TE = 0.53 * 10.12 + 0.12 * 24.7 + 0.36 * 43.9$
TE = 24.13 %
...
cumulative.tss.95 <- system.parameters.95 %>%
  ...
  mutate(cs = cumsum(process.tss),
         ...
         process.flow.st = abs(process.tss * 1e6 / (tss.c2.st - tss.c1.st) / 24 / 1000),
...
``` {r StFlowplot95, fig.cap = "95^th^ st. 2022 Settlement tank flow requirements to maintain 250 mg/l TSS (per System).", fig.align = "center", fig.dim = c(8,3), dpi = 200}

# plot drum filter flow requirement 95th 2022
st.flowplot.95 <- cumulative.tss.95 %>%
  filter(batch == "lag.batch.0" & !system %in% c("quarantine", "depurate")) %>%
  ggplot(aes(batch_day, process.flow.st, color = system)) +
  geom_line()+ 
  scale_color_manual("", breaks = c("nursery", "juvenile", "growout"),
                     values = c(system_cols),
                     labels = c("Nursery", "Juvenile", "Growout")) +
  labs(x = "Day", y = expression(paste("Settlement tank ", (m^3/hr)))) + 
  theme_minimal()

st.flowplot.95
```

``` {r DrumSettlementFlow_95}

# get max and average flowrates required for drum tank operation
drum.flow.process.95 <- cumulative.tss.95 %>%
  drop_na() %>%
  group_by(system) %>%
  summarise(drum.flow.p.max = max(process.drum.flow),
            drum.flow.p.avg = mean(process.drum.flow)) %>%
  arrange(factor(system, levels = c("quarantine", "nursery", "juvenile", "growout", "depurate")))

# get max and average flowrates required for settlement tank operation
st.flow.process.95 <- cumulative.tss.95 %>%
  drop_na() %>%
  group_by(system) %>%
  summarise(st.flow.p.max = max(process.flow.st),
            st.flow.p.avg = mean(process.flow.st)) %>%
  arrange(factor(system, levels = c("quarantine", "nursery", "juvenile", "growout", "depurate")))

# join flow rate summary tibbles 95th st 2022
solids.flow.summary.combined.95 <- left_join(drum.flow.process.95,
                                 st.flow.process.95,
                                 by = "system") %>%
  filter(!system %in% c("quarantine", "depurate"))
```

``` {r FacilitySolidsFlowTable95, ft.align = "center"}

solids.flow.summary.combined.95 %>%
  flextable() %>%
  set_header_labels(drum.flow.p.max = "Max Flow DF", drum.flow.p.avg = "Avg Flow DF", st.flow.p.max = "Max Flow ST", st.flow.p.avg = "Avg Flow ST") %>%
  colformat_double(digits = 2) %>%
  autofit() %>%
  fit_to_width(7.5) %>%
set_caption(caption = as_paragraph("95", as_sup("th"), " st. 2022 Estimated system drum filter (DF) and settlement tank (ST) required flowrates m", as_sup("3"), "/hr."), 
  style = "Table Caption", 
  autonum = run_autonum(seq_id = "tab"))
```
...
Drum filter water usage calculations based upon 50 litres consumed for cleaning per kilo of feed supplied using a 100 $\mu $m mesh (under normal fish farm conditions). Adjusted here to reflect additional bacterial biomass produced under biofloc conditions. For comparison clearwater drumfilter water use calculated based upon 200 litres consumed per kilo of feed using a 30 $\mu $m mesh. Note - solids profile does not seem to warrant a change of mesh size from 30 to 100 $\mu $m. Therefore water consumption rates could be a multiple of four times greater by utilising 30 $\mu $m mesh.
...
Air demand based upon aeration at atmospheric pressure and a transfer efficiency of 5%
...
``` {r molassesUse, , fig.cap = "Molasses inputs in RW 4 2022.", fig.align = "center", fig.dim = c(8,3), dpi = 200}
...
Comments: Information indicating that molasses added when solids level below 1ml in imhoff cone. Not consistent with requirement modelling above based on stoichiometry indicates addition of carbon source according to protein addition from feed.  
...
# 95^th^ st. 2023 Operation modelling
...
# 101^st^ st. 2024 Operation Modelling
